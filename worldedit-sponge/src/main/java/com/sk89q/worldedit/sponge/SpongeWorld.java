...
public class SpongeWorld extends LocalWorld {
    ...
    public List<com.sk89q.worldedit.entity.Entity> getEntities(Region region) {
        ...
        Collection<Entity> ents = world.getEntities();
        List<com.sk89q.worldedit.entity.Entity> entities = new ArrayList<com.sk89q.worldedit.entity.Entity>();
        for (Entity ent : ents) {
            if (region.contains(SpongeUtil.toVector(ent.getLocation()))) {
                entities.add(SpongeAdapter.adapt(ent));
            }
        }
        return entities;
    }
    ...
    public List<com.sk89q.worldedit.entity.Entity> getEntities() {
        ...
        for (Entity entity : getWorld().getEntities()) {
            list.add(SpongeAdapter.adapt(entity));
        }
        return list;
    }
    ...
    public int getBlockLightLevel(Vector pt) {
        return getWorld().getLuminance(pt.getBlockX(), pt.getBlockY(), pt.getBlockZ());
    }
    ...
    public boolean clearContainerBlockContents(Vector pt) {
        BlockState block = getWorld().getBlock(pt.getBlockX(), pt.getBlockY(), pt.getBlockZ());
        if (block == null) {
            return false;
        }
        block.getD
    ...
    public void dropItem(Vector pt, BaseItemStack item) {
        ...
        ItemStack bukkitItem = new ItemStack(item.getType(), item.getAmount(),
                item.getData());
        world.dropItemNaturally(SpongeUtil.toLocation(world, pt), bukkitItem);
    }
    ...
    public boolean setBiome(Vector2D position, BaseBiome biome) {
        ...
        if (adapter != null) {
            SpongeBiome spongeBiome = adapter.getBiome(biome.getId());
            getWorld().setBiome(position.getBlockX(), position.getBlockZ(), spongeBiome);
            return true;
        } else {
            return false;
        }
